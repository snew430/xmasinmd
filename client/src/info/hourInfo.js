const hour1 = [
  'Spirit of The Season - Alan Silvestri & Chorus',
  'Feature - Maryland"s Charlie Brown Christmas Tree',
  'Let it Snow, Let it Snow, Let it Snow! - Michael Buble',
  'Feature - A Williamsport Christmas',
];

const hour2 = [
  'Step Into Christmas - Elton John with John Waters Comments',
  'Feature - Crabs for Christmas at the McKenna"s',
  'Mistletoe and Holly - Frank Sinatra',
  'I Saw Mommy Kissing Santa Claus - Ronettes',
];

const hour3 = [
  'Happy Holidays/The Holiday Season - Andy Williams',
  'Little Saint Nick - Beach Boys',
  'Feature - Western Maryland Polar Express',
  'Believe - Josh Groban',
];

const hour4 = [
  'Winter Wonderland/Sleigh Ride - Dolly Parton',
  'Christmas Canon - Trans Siberian Orchestra',
  'Feature - Annapolis Gingerbread Village',
  'Holly Jolly Christmas - Burl Ives',
];

const hour5 = [
  'March Of The Toys - Fiedler/Boston Pops',
  'Santa Claus Is Coming To Town - Four Seasons',
  'Feature - Letters To Santa',
  'Santa Claus Is Watching You - Ray Stevens',
];

const hour6 = [
  'The Most Wonderful Time of the Year - Andy Williams',
  'Feature - Ravens At Christmastime - John Harbaugh,Joe Flacco plus Michael Phelps',
  'Feature - Steve Bisciotti Christmas Memories',
  "The Christmas Song - Nat 'King' Cole",
];

module.exports = { hour1, hour2, hour3, hour4, hour5, hour6 };
